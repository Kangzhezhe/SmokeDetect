...
void config(SPI_HandleTypeDef hspi, struct CsPin csPin) {
    ...
    while (writeToRegister(0x12, 0x0320, hspi, csPin, true) != HAL_OK); // 10 Hz  100mS //采样率 fs = 32k/(reg*4) Hz    数据手册P60
    while (writeToRegister(0x14, 0x011D, hspi, csPin, true) != HAL_OK);// 时隙A:LED1(蓝光), 时隙B:LED3(红光)        P47
